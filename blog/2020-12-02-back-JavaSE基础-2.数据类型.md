...
浮点类型:[浮点数=符号位+指数位+尾数位]
tip1:尾数部分可能丢失,会造成精度损失(小数都是近似值)
tip2:浮点常量默认是double,所以定义时必须指明,foalt后跟"F",double后跟"D"
tip3:常用double来定义小数的
tip4:避免使用`double x = 1.1F;` 这种跨类型定义方式,因为2种类型的范围是不同的,和期待的值有偏差
tip5:当我们对运算结果是小数的进行相等判断时,要小心!!!
 	float: 占4个字节(32位), 要在数字后加"F"标识
 	double: 占8个字节(64位), 要在数字后加"D"标识
